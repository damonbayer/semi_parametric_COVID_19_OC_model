...
# @model function bayes_seird(data_new_deaths, data_new_cases, tests, data_seroprev_cases, seroprev_tests, obstimes, seroprev_times, param_change_times, extra_ode_precision)
...
#   ϕ_deaths = exp(ϕ_deaths_non_centered * 0.46 + ϕ_deaths_non_centered_mean)
...
#     data_new_cases[i] ~ BetaBinomial2(tests[i], max(cases_pos_mean[i], 0.0), ϕ_cases_bb)
#   end
...
# @model function bayes_seird(data_new_deaths, data_new_cases, tests, data_seroprev_cases, seroprev_tests, obstimes, seroprev_times, param_change_times, use_tests, extra_ode_precision)
#   # Calculate number of observed datapoints timepoints
#   l_incidence = length(data_new_deaths)
#   l_prevalence = length(data_seroprev_cases)
#   l_param_change_times = length(param_change_times)
...
#     σ_α_non_centered = α_t_params_non_centered[1]
#   else
#     σ_ρ_cases_non_centered = ρ_cases_t_params_non_centered[1]
#   end
...
#     α_init_non_centered = α_t_params_non_centered[2]
#   else
#     ρ_cases_init_non_centered = ρ_cases_t_params_non_centered[2]
#   end
...
#     cases_pos_mean = logistic.(α_t_values_with_init .+ logit.(sol_new_cases / popsize))
#   else
#     cases_pos_mean = ρ_cases_t_values_with_init .* sol_new_cases
#   end
...
#       data_new_cases[i] ~ BetaBinomial2(tests[i], max(cases_pos_mean[i], 0.0), ϕ_cases_bb)
#     else
#       data_new_cases[i] ~ NegativeBinomial2(max(cases_pos_mean[i], 0.0), ϕ_cases_nb)
#     end
#   end
...
# @model function bayes_seird(data_new_deaths, data_new_cases, tests, data_seroprev_cases, seroprev_tests, obstimes, seroprev_times, param_change_times, use_tests, use_seroprev, extra_ode_precision)
#   # Calculate number of observed datapoints timepoints
#   l_incidence = length(data_new_deaths)
#   if use_seroprev
#     l_prevalence = length(data_seroprev_cases)
