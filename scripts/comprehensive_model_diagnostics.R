...
major_experiment_name <- "real_data_experiment"
experiment_results_path <- path("experiments", major_experiment_name, "results")

experiment_name <-
  path(experiment_results_path, "posterior_predictive") %>%
  dir_ls() %>%
  path_file() %>%
  path_ext_remove() %>%
  str_remove("posterior_predictive_") %>%
  pluck(1)

file_path_posterior_gq <- path(experiment_results_path, str_c("posterior_generated_quantities/posterior_generated_quantities_", experiment_name), ext = "csv")
file_path_prior_gq <- path(experiment_results_path, str_c("prior_generated_quantities/prior_generated_quantities_", experiment_name), ext = "csv")
file_path_posterior_predictive <- path(experiment_results_path, str_c("posterior_predictive/posterior_predictive_", experiment_name), ext = "csv")
file_path_posterior_samples <- path(experiment_results_path, str_c("posterior_samples/posterior_samples_", experiment_name), ext = "csv")
figure_path <- path(experiment_results_path, str_c("comprehensive_model_diagnostics_", experiment_name), ext = "pdf")
...
# for simulated data
# dat <-
#   read_csv("experiments/real_data_experiment/simulated_data.csv") %>%
...
lp_samples <- read_csv(file_path_posterior_samples, col_select = c(iteration, chain, lp))
...
summarized_results <-
  ...
  summarize_draws(rhat, ess_basic, ess_bulk, .cores = parallelly::availableCores())
